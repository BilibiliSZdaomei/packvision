...
- 驱动和上位机到底干嘛：[[26 Astra Pro 驱动与上位机说明]]
- 每台仓库电脑怎么出验收证据：[[27 现场试运行验收报告]]
...
- 新增电子秤适配接口：基础包手动兜底，支持 mock 和 RS232 串口电子秤，USB HID 作为后续插件。
- 新增 WMS/TMS 本地集成队列和轻量 HTTP 推送器：测量保存后自动写入本地 outbox，接口不在线也不影响继续测量。
- 新增设备 watchdog：统一判断相机枚举、实时流卡顿、模拟兜底和现场恢复动作。
- 新增现场试运行验收报告：把 readiness、实时工位、历史、复核、统计、电子秤和 WMS/TMS 状态合成可导出的证据链。
...
- [[24 WMS TMS 本地集成队列]]：本地 outbox 如何保存、查看、推送、导出以及未来对接 WMS/TMS。
- [[25 设备守护与现场恢复]]：相机未检测到、实时流卡住、模拟兜底时现场怎么恢复。
- [[26 Astra Pro 驱动与上位机说明]]：驱动安装证据、OrbbecViewer 用途、与 PackVision 的边界。
- [[27 现场试运行验收报告]]：每台仓库电脑如何导出可汇报、可追溯的试运行证据。
