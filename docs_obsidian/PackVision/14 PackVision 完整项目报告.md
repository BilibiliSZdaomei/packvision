...
> [!summary] 结论
> PackVision 已经从图片测量 Demo 推进为本地 DWS 仓库测量工作台雏形：主流程是 Astra Pro 实时监控、稳定后记录，图片版作为兜底；深度相机版已有接入和验收底座，海外仓交付包可生成并控制在 100 MB 内。
...
1. 扫码或录入单号。
2. 连接 Astra Pro 深度相机，查看顶部/正面/侧面多相机监控。
3. 系统自动测量包装尺寸，稳定后点击记录。
4. 标记异常材质和低置信风险。
5. 保存历史、标注图和测量结果。
6. 统计使用次数和测量次数。
7. 导出 CSV 给现场验收和管理汇报。
