...
| 实时采集监控 | 已完成并升级 | Astra Pro 实时流持续测量，按钮只负责记录稳定结果；默认实时工位按配置顺序采样 `top` + `front`，UI 显示多相机监控矩阵、FPS、运行时长、测量次数和采集后端。 |
| 双 Astra Pro 采集 | 已完成现场首验 | 当前电脑已配置 `top` + `front` 两台 Astra Pro；采集后端按 `serial_hint` 只打开目标设备，默认 live 不再走模拟兜底，避免假画面误导现场。 |
...
```text
D:\Documents\包装尺寸检测\release\PackVision_Field_Kit_20260528_2051.zip
```
