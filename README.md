...
| 双 Astra Pro 采集 | 已完成现场首验 | 当前电脑已配置 `top` + `front` 两台 Astra Pro；采集后端按 `serial_hint` 只打开目标设备，避免双相机时 OpenNI 同时打开全部设备导致崩溃。 |
...
```text
D:\Documents\包装尺寸检测\release\PackVision_Field_Kit_20260528_2032.zip
```
